...
function SprintCollaborators({
  ...
  return (
    <Dialog open={openCollaborators} onOpenChange={setOpenCollaborators}>
      <DialogContent className="max-h-[90vh] max-w-[500px] overflow-y-auto rounded-lg border border-gray-200 shadow-lg dark:border-gray-700">
        <DialogHeader>
        ...
        <div className="p-4">
          ...
          <div className="space-y-4">
            {filteredCollaborators.map((collaborator) => (
              <div
                key={collaborator.id}
                className="flex w-full items-center justify-between rounded-lg border border-gray-300 p-4 shadow-sm transition-shadow duration-200"
              >
                ...
                {collaborators.length > 1 && (
                  <Button
                    size={"icon"}
                    variant={"ghost"}
                    className="ml-4 rounded text-red-500 hover:text-red-600"
                  >
                    <Trash2 />
                  </Button>
                )}
          ...
          {joinCode && (
            <div className="mt-6 flex flex-col items-center justify-between rounded-lg border border-gray-300 p-6 shadow-sm transition-shadow duration-200">
